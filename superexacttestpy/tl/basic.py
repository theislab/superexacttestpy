...
def cpsets_sim(
    x: int, set_len: list, n: int, lower_tail: bool = True, logp: bool = False, number_sim: int = 10000
) -> float:
    """
...
def cpsets(
    ...
    logp: bool = False,
    p_val_sim: bool = False,
    number_sim: int = 100000,
) -> float:
    """
    ...
    for i in barcode:
        ...
        for range_code in range(nl):
            if i[range_code] == "1":
                ...
                regex += "[01]"
        for elem in el[el["barcode"].str.contains(regex)][
            "entry"
        ]:  # Search if the barcode match with the regex and get the entry name.
            tmp.append(elem)
        tmp = sorted(tmp)
    ...
    res = pd.DataFrame(
        {
            "intersection": decode,
            "degree": odegree,
            "observed_overlap": otab,
            "expected_overlap": etab,
            "fold_enrichment": FE,
            "p_value": p_val,
            "elements": elements,
        }
